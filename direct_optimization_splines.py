...
def generate_data(
    ...
    for i in range(number_of_trajectories):  # Number of trajectories to generate
        ...
        dmp_params2 = [dmp_x1.goal, dmp_y1.goal, dmp_x2.goal, dmp_y2.goal, spline2.curvature]
...
def train(initial_state,
    ...
    for epoch in range(num_epochs):
        ...
        dmp_params1 = net_goal()[:3]
        dmp_params2 = net_goal()[3:]

        # Force the start positions in dmp_params1 and dmp_params2, and center in home reference frame
        dmp_params1_positions = torch.cat([state[:2], dmp_params1[:2]])
        dmp_params2_positions = torch.cat([dmp_params1[:2] , dmp_params2[:2]])

        # Separate the position and DMP weights for clamping
        dmp_params1_weights = dmp_params1[2:]
        dmp_params2_weights = dmp_params2[2:]
        ...
        dmp_params1 = torch.cat([dmp_params1_positions, dmp_params1_weights]) + opt_noise*eps1
        dmp_params2 = torch.cat([dmp_params2_positions, dmp_params2_weights]) + opt_noise*eps2
        ...
        penalty_see_object = torch.relu(0.65 - final_state_preds1[0]) * torch.relu(0.5 - final_state_preds1[1])


        if task == "reach_final_goal":
            loss_goal = loss_goal1 + 0.25 * loss_coll1 + 10*penalty_see_object
        else:
    ...
    print(dmp_params1_no_noise)
    print(dmp_params2_no_noise)
...
if __name__ == "__main__":
    ...
    num_initial_prims = 5
    lower_bounds = torch.tensor([0.1,0.1,-1.0,0.1,0.1,-1.0])
    upper_bounds = torch.tensor([0.9,0.9,1.0,0.9,0.9,1.0])
    for i in range(num_initial_prims):
        initial_prim = torch.rand(6)*5-2.5
        net_goal = PrimitiveParams(initial_prim,lower_bounds,upper_bounds)
        net_preds = [copy.deepcopy(net), copy.deepcopy(net)]
        target_goal1 = [0.2,0.65]
        target_goal2 = [0.2,0.65]
