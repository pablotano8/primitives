...
def plot_pf_error_vs_n_trajectories(
        world, world_bounds, true_corners,
        n_values=(1, 2, 5, 10, 25, 50, 100, 200),
        n_seeds=8, waypoints_per_trajectory=5,
        pf_kwargs=None, title=None):
    """Sweep pre-training exploration size and plot estimator error vs N.

    For each N in n_values, runs `n_seeds` independent random-exploration +
    Gaussian-posterior pipelines, then plots:
        - L2(mean corners, true corners): bias of the estimate
        - mean posterior std (xL, xR): the agent's own uncertainty
    Both with median + 25-75 percentile band over seeds.
    """
    pf_kwargs = pf_kwargs or {}
    pf_kwargs.setdefault('wall_thickness', 0.2)
    ...
    for N in n_values:
        ...
        for seed in range(n_seeds):
            np.random.seed(seed)
            wp = random_exploration_waypoints(
                world, world_bounds, n_trajectories=N,
                waypoints_per_trajectory=waypoints_per_trajectory, verbose=False)
            pf = ParticleFilter(seed=seed, **pf_kwargs)
            update_pf_from_exploration(pf, true_corners, wp)
            s = pf.summary(true_corners)
            l2s.append(s['l2_to_truth'])
            stds.append(float(s['std_params'].mean()))
        results['L2'].append(l2s)
        results['std'].append(stds)
        print(f'N={N:>4}: L2 median={np.median(l2s):.4f}, std median={np.median(stds):.4f}')

    n_arr = np.array(n_values)
    L2 = np.array(results['L2'])      # (len(n_values), n_seeds)
    S = np.array(results['std'])

    fig, ax = plt.subplots(figsize=(7, 5))
    for arr, label, color in [(L2, 'L2(mean corners, truth)', 'tab:red'),
                              (S, 'mean posterior std', 'tab:blue')]:
        med = np.median(arr, axis=1)
        lo = np.percentile(arr, 25, axis=1)
        hi = np.percentile(arr, 75, axis=1)
        ax.plot(n_arr, med, '-o', color=color, label=label)
        ax.fill_between(n_arr, lo, hi, color=color, alpha=0.2)
    # 1/sqrt(N) reference line, scaled to median std at smallest N
    ref = S.mean(axis=1)[0] * np.sqrt(n_arr[0] / n_arr)
    ax.plot(n_arr, ref, '--', color='gray', alpha=0.6, label=r'$1/\sqrt{N}$ reference')

    # ax.set_xscale('log'); ax.set_yscale('log')
    ax.set_xlabel('# random exploration trajectories')
    ax.set_ylabel('error / posterior std')
    ax.set_title(title or f'PF error vs pre-training exploration size '
                          f'({n_seeds} seeds, {waypoints_per_trajectory} waypoints/trajectory)')
...
def plot_wm_error_vs_pf_quality(
        world, world_bounds, true_corners, net,
        n_values=(1, 2, 5, 10, 25, 50, 100, 200),
        n_seeds=5, n_test=200, waypoints_per_trajectory=5,
        complexity=1.0, pf_kwargs=None):
    """How well does the world model predict when fed wall corners sampled
    from a PF that's only been trained on N exploration trajectories?

    Builds a fixed test set of (start, dmp_params, true_final_pos) by running
    actual simulator rollouts. Then for each N:
      - build a PF with N pre-training trajectories
      - sample wall corners from that PF
      - feed the resulting s_t into the world model
      - compare WM-predicted final position to the simulator's actual one

    Produces two plots: (1) WM error vs N (matches plot_pf_error_vs_n_trajectories
    in shape), (2) WM error vs PF L2 error (the direct sensitivity curve).
    """
    pf_kwargs = pf_kwargs or {}
    pf_kwargs.setdefault('wall_thickness', 0.2)

    # 1. Fixed test set: random rollouts on the TRUE world.
    rng = np.random.default_rng(123)
    starts, dmp_params_list, true_finals = [], [], []
    for _ in range(n_test):
        world.reset()
        start, _ = generate_random_positions(world, world_bounds=world_bounds,
                                              orientation=None, circular=True)
        _, goal = generate_random_positions(world, world_bounds=world_bounds, orientation=None)
        c = complexity * ((abs(start[0] - goal[0]) + abs(start[1] - goal[1])) / 2)
        dmp_x = DMP1D(start=start[0], goal=goal[0], n_basis=3, complexity=c)
        dmp_y = DMP1D(start=start[1], goal=goal[1], n_basis=3, complexity=c)
        sim = Simulation(world, dmp_x, dmp_y, start, T=1.0, dt=0.01)
        positions, _, _, _, _ = sim.run()
        starts.append(np.asarray(start, dtype=np.float32))
        dmp_params_list.append([dmp_x.start, dmp_y.start, dmp_x.goal, dmp_y.goal,
                                *dmp_x.weights, *dmp_y.weights])
        true_finals.append(np.asarray(positions[-1], dtype=np.float32))
    starts = np.stack(starts)                       # (n_test, 2)
    dmp_params_t = torch.tensor(dmp_params_list, dtype=torch.float32)   # (n_test, 10)
    true_finals = np.stack(true_finals)             # (n_test, 2)

    # 2. Baseline: WM fed the TRUE corners (best the WM can do on this test set).
    net.eval()
    true_dists = np.linalg.norm(starts[:, None, :] - true_corners[None, :, :], axis=-1)
    s_t_true = np.concatenate([starts, np.ones((n_test, 1), np.float32),
                               true_dists.astype(np.float32)], axis=-1)
    with torch.no_grad():
        inp_true = torch.cat([torch.tensor(s_t_true, dtype=torch.float32), dmp_params_t],
                              dim=-1).unsqueeze(1)
        out_true, _ = net(inp_true)
        pred_true = out_true.squeeze(1)[:, :2].numpy()
    baseline_err = np.linalg.norm(pred_true - true_finals, axis=-1).mean()
    baseline_mae = np.mean(np.abs(pred_true - true_finals))
    print(f'WM baseline (true corners): Euclidean err={baseline_err:.4f}, '
          f'per-axis MAE={baseline_mae:.4f}  '
          f'(this is what train_predictive_net reports)')

    # 3. Sweep N. For each (N, seed), build PF, sample corners, score WM.
    wm_errs_per_N, pf_l2s_per_N, added_per_N = [], [], []
    pf_l2_all, wm_err_all, added_all, N_all = [], [], [], []
    for N in n_values:
        wm_errs, pf_l2s, addeds = [], [], []
        for seed in range(n_seeds):
            np.random.seed(seed)
            wp = random_exploration_waypoints(
                world, world_bounds, n_trajectories=N,
            ...
            pf = ParticleFilter(seed=seed, **pf_kwargs)
            update_pf_from_exploration(pf, true_corners, wp)
            pf_l2 = pf.summary(true_corners)['l2_to_truth']
            pf_l2s.append(pf_l2)

            sampled_corners = pf.sample(n_test)                  # (n_test, 4, 2)
            dists = np.linalg.norm(starts[:, None, :] - sampled_corners, axis=-1)
            s_t = np.concatenate([starts, np.ones((n_test, 1), np.float32),
                                  dists.astype(np.float32)], axis=-1)

            with torch.no_grad():
                inp = torch.cat([torch.tensor(s_t, dtype=torch.float32), dmp_params_t],
                                 dim=-1).unsqueeze(1)
                out, _ = net(inp)
                pred_final = out.squeeze(1)[:, :2].numpy()
            err = np.linalg.norm(pred_final - true_finals, axis=-1).mean()
            added = err - baseline_err
            wm_errs.append(err); addeds.append(added)

            pf_l2_all.append(pf_l2); wm_err_all.append(err)
            added_all.append(added); N_all.append(N)
        wm_errs_per_N.append(wm_errs); pf_l2s_per_N.append(pf_l2s); added_per_N.append(addeds)
        print(f'N={N:>4}: PF L2 median={np.median(pf_l2s):.4f}, '
              f'WM err median={np.median(wm_errs):.4f}, '
              f'added by PF noise={np.median(addeds):+.4f}')

    n_arr = np.array(n_values)
    WM = np.array(wm_errs_per_N)

    fig, ax = plt.subplots(figsize=(7, 5))
    med = np.median(WM, axis=1)
    lo = np.percentile(WM, 25, axis=1)
    hi = np.percentile(WM, 75, axis=1)
    ax.plot(n_arr, med, '-o', color='tab:purple',
            label='WM error (PF-sampled wall)')
    ax.fill_between(n_arr, lo, hi, color='tab:purple', alpha=0.2)
    ax.axhline(baseline_err, color='gray', linestyle='--',
               label=f'WM error w/ true wall ({baseline_err:.4f})')
    # ax.set_xscale('log')
    ax.set_xlabel('# pre-training exploration trajectories')
    ax.set_ylabel('||predicted final pos - actual final pos||')
    ax.set_title('World-model prediction error vs pre-training size')
    ax.grid(True, which='both', alpha=0.3)
    ax.legend()
    plt.tight_layout()
    plt.show()
    return {'n_values': list(n_values), 'wm_err': WM, 'baseline_err': baseline_err}
...
if __name__ == "__main__":
    ...
    pf = ParticleFilter(n_particles=10, wall_thickness=pf_world.wall_thickness,
                        x_left_range=(-1.0, 0.0), x_right_range=(0.0, 1.0), seed=0)

    # Sweep: how does estimator error scale with pre-training exploration size?
    plot_pf_error_vs_n_trajectories(
        pf_world, world_bounds, true_corners_train,
        n_values=(1, 2, 5, 10, 25, 50, 100, 200),
        n_seeds=8, waypoints_per_trajectory=5,
        pf_kwargs={'wall_thickness': pf_world.wall_thickness,
                   'x_left_range': (-1.0, 0.0), 'x_right_range': (0.0, 1.0)})

    # Sweep: how does world-model prediction error scale with PF state-estimation error?
    plot_wm_error_vs_pf_quality(
        pf_world, world_bounds, true_corners_train, net,
        n_values=(1, 2, 3, 4, 5, 6, 7,8,9),
        n_seeds=5, n_test=200, waypoints_per_trajectory=10,
        pf_kwargs={'wall_thickness': pf_world.wall_thickness,
                   'x_left_range': (-1.0, 0.0), 'x_right_range': (0.0, 1.0)})
    ...
    explo_waypoints = random_exploration_waypoints(
        pf_world, world_bounds, n_trajectories=2, complexity=1.5)
