        ...
        if not stop_training:
            ...
            for i in range(0, len(train_data)-batch_size,batch_size):
                ...
                if task == "reach_final_goal":
                    ...
                    loss_goal =-criterion(batch_s_t, final_state_preds1) -criterion(final_state_preds2, final_state_preds1) -criterion(final_state_preds1, batch_s_t)
                elif task == "goal_and_return":
                    target_goal_batch = torch.stack([torch.tensor(target_goal2) for item in batch])
                    loss_goal =  criterion(final_state_preds1, target_goal_batch) + criterion(final_state_preds2, batch_s_t)
                else:
        ...
        if epoch%eval_freq == 0:
            ...
            with torch.no_grad():
                ...
                if task == "reach_final_goal" or task=='use_rl':
                    ...
                    loss = np.mean(np.abs(np.array(actual_final_position2)- np.array(batch_s_t))) + np.mean(np.abs(np.array(actual_final_position2)- np.array(actual_final_position1)))  + np.mean(np.abs(np.array(actual_final_position1)- np.array(batch_s_t)))
                    loss = loss / 1.1
...
if __name__ == "__main__":
    ...
    valid_loss_task = {'reach_two_goals':[]  }
