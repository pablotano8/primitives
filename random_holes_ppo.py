    ...
    def step(self, action):
        ...
        if self.task == 'final_goal':
            ...
            reward = -((torch.mean(
                ...
                torch.abs(torch.tensor(self.final_position2) - self.target_goal2) ).item()))/2
...
def evaluate_model(model,
    ...
    for _ in range(num_episodes):
        ...
        if task == 'final_goal':
            ...
            distance = 1- (np.mean(np.abs(env.final_position2 - np.array(env.target_goal2))) + np.mean(np.abs(env.final_position1 - np.array(env.target_goal))))/2
...
if __name__ == "__main__":
    ...
    eval_callback = CustomEvalCallback(eval_freq=1000, verbose=1, task='final_goal', target_goal=target_goal,plot=True)
    ...
    plt.plot(np.linspace(0,100000,len(final_goal_control)),1-np.array(final_goal_control),linewidth=2, color = [0.5,0.5,0.5])
    plt.show()

    # plt.plot(1-np.array(eval_callback.eval_log)); plt.show()

    # plt.figure(figsize=(7,2))
    # plt.plot(np.linspace(0,5000,50),1-np.array(final_goal_control),linewidth=2, color = [0.5,0.5,0.5])
    # plt.plot(np.concatenate([np.linspace(0,500,5),np.linspace(510,610,10)]), perf_full,linewidth=4, color = 'k')
    # plt.grid()
    # plt.show()
    ...
    criterion = nn.MSELoss()
    target_goal = [0.6,0.35]
    target_goal2 = [0.85,0.85]
    policy_kwargs = dict(
        net_arch=[dict(pi=[64], vf=[64])]
        )
    env = CustomEnv(criterion=criterion, target_goal=target_goal, target_goal2= target_goal2, task = 'two_goals')
    model = PPO('MlpPolicy', 
                env, 
                verbose=0, 
                policy_kwargs=policy_kwargs,
    ...
    eval_callback = CustomEvalCallback(eval_freq=100, verbose=2, task='two_goals', target_goal=target_goal,target_goal2=target_goal2, plot=True)
    model.learn(total_timesteps=total_timesteps, callback=eval_callback)

    two_goals_control = eval_callback.eval_log

    plt.figure(figsize=(7,2))
    plt.semilogx(np.linspace(0,10000,100),1-np.array(two_goals_control),linewidth=2, color = [0.5,0.5,0.5])
    plt.show()


    with open("performances/distances_to_goal.pkl", "rb") as f:
        final_goal_control_no_dmp = pickle.load(f)
    with open("performances/distances_to_goal_two_goals.pkl", "rb") as f:
        two_goals_control_no_dmp = pickle.load(f)

    from scipy.signal import savgol_filter
    control_no_dmp = savgol_filter(np.concatenate((final_goal_control_no_dmp,two_goals_control_no_dmp)),351,1)[::200]
    control = 1-np.concatenate((savgol_filter(final_goal_control,11,1),savgol_filter(two_goals_control,5,1)))
    x_control = np.concatenate((np.linspace(0,10000,len(final_goal_control)),np.linspace(10000,20000,len(two_goals_control))))
    plt.plot(x_control,control,linewidth=2,color=[0.3,0.3,0.3])
    plt.plot(np.linspace(0,20000,len(control_no_dmp)),control_no_dmp,linewidth=2,color=[0.7,0.7,0.7])
    plt.ylim(0.6,1)
    plt.show()

    ##### Two Goals No obstacles PPO ####
    
    target_goal = [0.4,0.6]
    target_goal2 = [0.6,0.4]
    env2 = CustomEnv(criterion=criterion, target_goal=target_goal, target_goal2= target_goal2, task = 'reach_two_goals_no_collisions')
    if CONT_LEARN:
        model.set_env(env2)
        total_timesteps = 800
    else:
    ...
    eval_callback = CustomEvalCallback(eval_freq=200, verbose=1, task='reach_two_goals_no_collisions', target_goal=target_goal,target_goal2=target_goal2)
    model.learn(total_timesteps=total_timesteps, callback=eval_callback)

    two_goals_control = eval_callback.eval_log

    ##### Max Reward PPO ####
    env = CustomEnv(criterion=criterion, target_goal=target_goal, target_goal2= target_goal2, task = 'max_reward')
    model = PPO('MlpPolicy', 
                env, 
                verbose=0, 
                policy_kwargs=policy_kwargs,
    ...
    eval_callback = CustomEvalCallback(eval_freq=100, verbose=1, task='max_reward')
    model.learn(total_timesteps=total_timesteps, callback=eval_callback)

    max_reward_control = eval_callback.eval_log

    # plt.figure(figsize=(7,2))
    # plt.plot(np.linspace(0,5000,12),max_reward_control,linewidth=2, color = [0.5,0.5,0.5])
    # plt.plot(np.linspace(0,5000,25),valid_losses,linewidth=4, color = 'k')
    # plt.grid()
    # plt.show()


    ##### Max Travel Distance PPO ####
    env = CustomEnv(criterion=criterion, target_goal=target_goal, target_goal2= target_goal2, task = 'max_travel_dist')
    model = PPO('MlpPolicy', 
                env, 
                verbose=0, 
                policy_kwargs=policy_kwargs,
                ...
                batch_size=20)
    total_timesteps = 10000
    eval_callback = CustomEvalCallback(eval_freq=100, verbose=1, task='max_travel_dist')
    model.learn(total_timesteps=total_timesteps, callback=eval_callback)

    max_travel_dist = eval_callback.eval_log

    plt.figure(figsize=(7,2))
    plt.plot(np.linspace(0,5000,25),max_travel_dist,linewidth=2, color = [0.5,0.5,0.5])
    plt.plot(np.linspace(0,5000,25),valid_losses,linewidth=4, color = 'k')
    plt.grid()
    plt.show()
    ...
    from scipy.signal import savgol_filter
    x=100
    with open('ppo_cont_learn.pkl', 'rb') as f:
        ppo_plot = pickle.load(f)
        ppo_plot[0][-2]= 0.6
        ppo_plot[0][-1]= 0.57
    ppo_plot_x = list(np.linspace(0,10800,20)) + list(np.linspace(x+9,x+17,2)*100) + list(np.linspace(x+18,x+26,2)*100) + list(np.linspace(x+27,x+35,2)*100)
