...
from continuous_nav_envs import generate_random_positions, RandomRectangleWorld
...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        dmp_params1_positions = torch.cat([torch.Tensor(self.start_position), dmp_params1[0][2:4] ])
        ...
        final_position1, final_position2, collision_info = generate_trajectories_from_dmp_params(
            ...
            batch_size=1,
            batch_s_t=np.array(torch.tensor([self.start_position])),
            world=self.world,
            circular=True,
            n_basis=3)  # add batch dimension
        
        self.initial_pos = torch.Tensor(self.start_position).squeeze()
        ...
        if self.task == 'final_goal':
            ...
            reward = criterion(self.final_position2, self.initial_pos).item()
    ...
    def reset(self):
        if self.world is None:
            self.world = RandomRectangleWorld(
                ...
                friction=0)
        self.world.reset()
        self.start_position = generate_random_positions(self.world, world_bounds=world_bounds,circular = True)[0]
        wall_length = self.world.radius * 2 * self.world.wall_size # Wall length
        wall_shape = [(-wall_length / 2, -self.world.wall_thickness / 2), (wall_length / 2, -self.world.wall_thickness / 2),
                    (wall_length / 2, self.world.wall_thickness / 2), (-wall_length / 2, self.world.wall_thickness / 2)]
        if not self.world.wall_present:
        ...
        self.s_t =  np.concatenate((self.start_position,np.array([self.world.wall_present*1.0]),distances_to_edges))
...
def evaluate_model(model,
    ...
    for _ in range(num_episodes):
        ...
        if task == 'final_goal':
            ...
            performance = np.mean(np.linalg.norm(np.array(env.final_position2.numpy()) - np.array(env.start_position))) / (1.66)
...
def plot_trajectories(model,
    ...
    world.reset()
    obs = env.reset()
    initial_position = torch.Tensor([env.s_t[:2]])
    action, _ = model.predict(obs)
    _, _, _, _ = env.step(action)
    ...
    # Force the start positions in dmp_params1 and dmp_params2, and center in home reference frame
    dmp_params1_positions = torch.cat([torch.Tensor(env.s_t[:2]), dmp_params1[0][2:4] ])
    dmp_params2_positions = torch.cat([dmp_params1[0][2:4] , dmp_params2[0][2:4]])
    ...
    generate_and_plot_trajectories_from_parameters(dmp_params1, dmp_params2, 1, initial_position, world, world_bounds, n_basis=3, circular=True)
...
##### Escaping Rat ####
    ...
    env = CustomEnv(criterion=criterion, target_goal=target_goal, task = 'final_goal',world=world)
    ...
    world = RandomRectangleWorld(num_obstacles=0, max_speed=100, radius=1, wall_present=True,wall_size=0.55,friction=0)
    eval_callback = CustomEvalCallback(eval_freq=1000, verbose=2, task='final_goal', target_goal=target_goal, world=world,plot=True)
    total_timesteps = 500000
    model.learn(total_timesteps=total_timesteps, callback=eval_callback)

    performance = eval_callback.eval_log

    with open("performances/rand_obs_ppo_one_env_max_distance.pkl", "wb") as f:
